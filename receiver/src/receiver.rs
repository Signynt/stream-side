...
//         loop {
//             // 1. Читаем длину сериализованного VideoPacket
...
//                 eprintln!("⚠️ Странная длина пакета: {}", len);
//                 break; 
//             }
...
//             // 3. Десериализация через postcard
//             let packet: VideoPacket = match postcard::from_bytes(&frame_buf) {
//                 Ok(p) => p,
//                 Err(e) => {
...
//             // 5. Кормим FFmpeg только payload-ом (там чистый HEVC)
//             let mut ffmpeg_packet = ffmpeg_next::Packet::new(packet.payload.len());
//             if let Some(dst) = ffmpeg_packet.data_mut() {
//                 dst.copy_from_slice(&packet.payload);
//             }

//             if let Err(e) = decoder.send_packet(&ffmpeg_packet) {
//                 eprintln!("❌ Ошибка декодера: {:?}", e);
//                 continue;
//             }
...
//                 // Проверка/создание скейлера (в YUV420P для шейдера)
//                 let fmt = raw.format();
//                 let (w, h) = (raw.width(), raw.height());

//                 if fmt != last_fmt {
//                     last_fmt = fmt;
//                     scaler = if fmt == Pixel::YUV420P { None } else {
//                         Some(scaling::Context::get(fmt, w, h, Pixel::YUV420P, w, h, scaling::Flags::BILINEAR).unwrap())
//                     };
//                 }

//                 // Конвертация и отправка в рендер
//                 let yuv420 = if let Some(ref mut sc) = scaler {
//                     let mut converted = Video::new(Pixel::YUV420P, w, h);
//                     sc.run(&raw, &mut converted).unwrap();
//                     converted
//                 } else {
//                     // Если уже YUV420P, копируем данные плоскостей
//                     let mut copy = Video::new(Pixel::YUV420P, w, h);
//                     for i in 0..3 {
//                         let s = raw.stride(i);
//                         let plane_h = if i == 0 { h } else { h / 2 };
//                         copy.data_mut(i)[..s * plane_h as usize].copy_from_slice(&raw.data(i)[..s * plane_h as usize]);
//                     }
//                     copy
//                 };
...
//                     width: w,
//                     height: h,
//                     y: yuv420.data(0)[..(yuv420.stride(0) * h as usize)].to_vec(),
//                     u: yuv420.data(1)[..(yuv420.stride(1) * h as usize / 2)].to_vec(),
//                     v: yuv420.data(2)[..(yuv420.stride(2) * h as usize / 2)].to_vec(),
//                     y_stride: yuv420.stride(0) as u32,
//                     u_stride: yuv420.stride(1) as u32,
//                     v_stride: yuv420.stride(2) as u32,
//                 };
